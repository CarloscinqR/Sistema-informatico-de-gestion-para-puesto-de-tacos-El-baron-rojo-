...
class interfacesProducto():
    ...
    def nuevoProducto(self, nuevo_producto):
        ...
        fondo2.pack(padx=50, pady=30, fill="both", expand=True)

        lbl_titulo = Label(fondo2, text="Nuevo producto",
                        font=("Orelega One", 40), fg="#F1C045", bg="#A6171C")
        lbl_titulo.pack(pady=20)

        fondo3 = Frame(fondo2, bg="white")
        fondo3.pack(expand=True, fill="both", padx=20, pady=20)

        # --- Nombre y precio centrados ---
        Label(fondo3, text="Nombre del producto", font=("Inter", 20), bg="white").pack(pady=5)
        nombre_entry = Entry(fondo3, font=("Inter", 20), bg="white", width=30, justify="center")
        nombre_entry.pack(pady=5)

        Label(fondo3, text="Precio unitario", font=("Inter", 20), bg="white").pack(pady=5)
        precio_entry = Entry(fondo3, font=("Inter", 20), bg="white", width=15, justify="center")
        precio_entry.pack(pady=5)

        # --- Ingredientes y cantidades en fila horizontal ---
        fila_ingredientes = Frame(fondo3, bg="white")
        fila_ingredientes.pack(pady=10, fill="x")

        Label(fila_ingredientes, text="Ingredientes", font=("Inter", 20), bg="white").grid(row=0, column=0, sticky="nw")
        lista_ing = Listbox(fila_ingredientes, selectmode="multiple", font=("Inter", 18), height=8, bg="white", width=25)
        ingredientes = ["Queso", "Tomate", "Lechuga", "Carne", "Pan", "Cebolla", "Salsa"]
        for ing in ingredientes:
            lista_ing.insert(END, ing)
        lista_ing.grid(row=1, column=0, sticky="nw", padx=(0, 20))

        contenedor_cantidades = Frame(fila_ingredientes, bg="white")
        contenedor_cantidades.grid(row=1, column=1, sticky="nw")

        self.ingredientes_entries = {}

        btn_add_ing = Button(fondo3, text="Agregar cantidades", font=("Inter", 18), bg="#F1C045",
            command=lambda: [
                [w.destroy() for w in contenedor_cantidades.winfo_children()],
                [
                    (
                        Label(contenedor_cantidades, text=f"{lista_ing.get(i)}:", font=("Inter", 18), bg="white")
                        .grid(row=idx, column=0, sticky="w", pady=3),
                        self.ingredientes_entries.update({
                            lista_ing.get(i): Entry(contenedor_cantidades, font=("Inter", 18), width=8, bg="#F1F0EE")
                        }),
                        self.ingredientes_entries[lista_ing.get(i)].grid(row=idx, column=1, sticky="w", padx=5)
                    )
                    for idx, i in enumerate(lista_ing.curselection())
                ]
            ]
        )
        btn_add_ing.pack(pady=10, anchor="w")

        # --- Botones finales alineados a la derecha en fila horizontal ---
        fila_botones = Frame(fondo3, bg="white")
        fila_botones.pack(fill="x", pady=10)

        btn_regresar = Button(fila_botones, text="Regresar", font=("Inter", 20), bg="#F1C045",
                            command=lambda: self.menu_producto(nuevo_producto))
        btn_regresar.pack(side=RIGHT, padx=10)

        btn_agregar = Button(fila_botones, text="Guardar producto", font=("Inter", 20), bg="#F1C045")
